...
export const ExplorePage = ({
  ...
  if (selectedCoin) {
    return <CoinDetailPage coin={selectedCoin} onBack={() => setSelectedCoin(null)} onProfileRedirect={onProfileClick} />;
  }
  if (showCreatePod) {
    return <CreatePodPage onBack={() => setShowCreatePod(false)} />;
  }
  return <div className="min-h-screen bg-gray-50 relative">
      ...
      <div className="flex items-center justify-between p-4 pt-12">
        <button onClick={onProfileClick} className="w-12 h-12 rounded-full overflow-hidden">
          <img src="/lovable-uploads/7179a805-1e9a-4986-bf99-5e78cc2b0caa.png" alt="Profile" className="w-full h-full object-cover" />
        </button>
      ...
      <div className="px-4 mb-6">
        <div className="rounded-3xl p-8 text-white relative overflow-hidden text-center" style={{
        background: 'radial-gradient(circle at 0% 0%, #79b5fd, #a659ef, #f372b7, #f77518)'
      }}>
          <div className="relative z-10">
            <h2 className="font-bold mb-2 text-2xl">Goals to Save Today</h2>
            <p className="text-sm opacity-90 mb-6">What do you want to achieve?</p>
            <button onClick={() => setShowGoalsSearch(true)} className="bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl px-6 py-3 flex items-center justify-center space-x-2 hover:bg-opacity-30 transition-all mx-auto">
      ...
      <div className="px-4 space-y-3 pb-24">
        {allPods.map((crypto, index) => <div key={index} onClick={() => setSelectedCoin(crypto)}>
            <CryptoItem {...crypto} />
          </div>)}
      </div>

      {/* Floating Action Button - Black */}
      <button onClick={() => setShowCreatePod(true)} className="fixed bottom-6 right-6 w-14 h-14 bg-black rounded-full shadow-lg flex items-center justify-center hover:bg-gray-800 transition-colors z-50">
      ...
      {showGoalsSearch && <GoalsSearchModal onClose={() => setShowGoalsSearch(false)} />}
    </div>;
};