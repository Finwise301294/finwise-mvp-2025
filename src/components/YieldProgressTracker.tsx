...
export const YieldProgressTracker = ({
  ...
  const getNextUnlock = () => {
    if (daysLocked < 8) return {
      days: 8 - daysLocked,
      rate: 1
    };
    if (daysLocked < 15) return {
      days: 15 - daysLocked,
      rate: 3
    };
    if (daysLocked < 30) return {
      days: 30 - daysLocked,
      rate: 6
    };
    return null;
  ...
  return <div className="bg-white rounded-2xl p-4 mb-4">
      ...
      <div className="relative mb-6">
        <div className="flex justify-between items-center">
          {milestones.map((milestone, index) => <div key={index} className="flex flex-col items-center">
              <div className={`w-4 h-4 rounded-full ${daysLocked >= milestone.days ? 'bg-success' : 'bg-gray-300'}`}></div>
        ...
        <div className="absolute top-2 left-0 right-0 h-0.5 bg-gray-200 -z-10">
          <div className="h-full bg-success transition-all duration-500" style={{
          width: `${Math.min(daysLocked / 30 * 100, 100)}%`
        }}></div>
        </div>
      </div>
      ...
      <div className="bg-gray-50 rounded-2xl p-4">
        ...
        {nextUnlock && <div className="mt-3 p-3 rounded-xl bg-green-200">
            <p className="text-sm text-emerald-600">
              Unlock {nextUnlock.rate}% yield in {nextUnlock.days} more days!
            </p>
          </div>}

        {daysLocked >= 30 && <div className="mt-3 p-3 bg-purple-50 rounded-xl">
